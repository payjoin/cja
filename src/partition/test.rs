...
use super::*;
use filters::{SubsetSumsFilter,PartitionsSubsetSumsFilter};
use types::{Set,Partition,Run};
...
fn regression_test_sum_filtered_partition_iterator() {
    ...
    for file in test_files {
        ...
        for (in_partition, out_partition) in run.partition_tuples {
            counter += 1;
            assert!(
                partition_tuples.iter().any(|&(ref in_p, ref out_p)| partition_eq(&in_partition, in_p) && partition_eq(&out_partition, out_p)),
...
fn partition_eq(part_a: &Partition, part_b: &Partition) -> bool {
    part_a.iter().all(|set_a| part_b.iter().any(|set_b| set_eq(set_a, set_b) ))
}

fn set_eq(set_a: &Set, set_b: &Set) -> bool {
    set_a.iter().all(|element_a| set_b.iter().any(|element_b| element_a == element_b))
}
...
fn test_tuple_iterator() {
    ...
    assert_eq!(
        ...
        vec![ (vec![1,3], vec![2]), (vec![1,2], vec![3]), (vec![1], vec![2, 3])])
}
