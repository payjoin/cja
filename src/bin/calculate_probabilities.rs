...
fn main() {
    ...
    print!("\tinput_output_zeros\tinput_output_ones\tinput_output_average_other\tinput_output_average");
    ...
    for run in result {
        ...
        {
            let (zeros, ones, average_other, average) = aggregated_in_out_probability(&run.in_coins, &run.out_coins, &non_derived_partitions);
            print!("\t{}\t{}\t{:.3}\t{:.3}", zeros, ones, average_other, average);
        };
        {
            let (zeros, ones, average_other, average) = aggregated_in_in_probability(&run.in_coins, &non_derived_partitions);
            print!("\t{}\t{}\t{:.3}\t{:.3}", zeros, ones, average_other, average);
        };
        {
            let (zeros, ones, average_other, average) = aggregated_out_out_probability(&run.out_coins, &non_derived_partitions);
            print!("\t{}\t{}\t{:.3}\t{:.3}", zeros, ones, average_other, average);
        };
        println!("")
    }
...
fn filter_derived_partitions(partitions: &Vec<(Partition, Partition)>) -> Vec<(Partition, Partition)> {
    let max_index = partitions.iter().map(|&(ref in_p, _)| in_p.len() ).max().unwrap();
...
fn is_derived(part: &(Partition, Partition), plus_part: &(Partition, Partition)) -> bool {
    ...
    for in_set in in_partition {
        let mtch = {
            plus_in_partition.iter().position(|set| in_set.iter().sum::<u64>() == set.iter().sum::<u64>())
        };
        match mtch {
            Some(i) => {plus_in_partition.remove(i); },
...
fn test_is_derived() {
    assert!(is_derived(
        &(vec![vec![1,2,3], vec![3,4]], vec![vec![1,2,3], vec![3,4]]),
        &(vec![vec![1,2], vec![3], vec![3,4]], vec![vec![1,2], vec![3], vec![3,4]])
    ));
    assert!(is_derived(
        &(vec![vec![2,2,3], vec![3,4]], vec![vec![2,2,3], vec![3,4]]),
        &(vec![vec![3,2], vec![2], vec![3,4]], vec![vec![3,2], vec![2], vec![3,4]])
    ))
}
...
fn aggregate_probabilities(probabilities: &Vec<f64>) -> (f64, f64, f64, f64) {
    ...
    let other: Vec<f64> = probabilities.iter().filter(|&&p| p > 0f64 && p < 1f64).map(|&e| e).collect();
    let average_other = average(&other);
    let average = average(probabilities);
    (zeros, ones, average_other, average)
}

fn in_out_probability(in_coin: &u64, out_coin: &u64, partition_tuples: &Vec<(Partition, Partition)>) -> f64 {
    partition_tuples.iter().filter(|&&(ref in_partition, ref out_partition)| {
        let in_set = match in_partition.iter().find(|set| set.iter().find(|&coin| coin == in_coin).is_some() ) {
            Some(set) => set,
            None => panic!("Did not find in coin in partition")
        };
        let out_set = match out_partition.iter().find(|set| set.iter().find(|&coin| coin == out_coin).is_some() ) {
            Some(set) => set,
            None => panic!("Did not find out coin in partition")
        };
        in_set.iter().sum::<u64>() == out_set.iter().sum::<u64>()
    }).count() as f64 / partition_tuples.len() as f64
}

fn aggregated_in_out_probability(in_coins: &Vec<u64>, out_coins: &Vec<u64>, partition_tuples: &Vec<(Partition, Partition)>) -> (f64, f64, f64, f64) {
    let probabilities: Vec<f64> = in_coins
        .iter()
        .flat_map(|in_coin| {
            out_coins.iter().map(move |out_coin| {
                in_out_probability(in_coin, out_coin, partition_tuples)
            })
        }).collect();
    aggregate_probabilities(&probabilities)
}

fn in_in_probability(first_in_coin: &u64, second_in_coin: &u64, partition_tuples: &Vec<(Partition, Partition)>) -> f64 {
    partition_tuples.iter().filter(|&&(ref in_partition, _)| {
        in_partition.iter().find(|set| {
...
fn aggregated_in_in_probability(in_coins: &Vec<u64>, partition_tuples: &Vec<(Partition, Partition)>) -> (f64, f64, f64, f64) {
    let probabilities: Vec<f64> = in_coins
        .iter()
        .enumerate()
...
fn out_out_probability(first_out_coin: &u64, second_out_coin: &u64, partition_tuples: &Vec<(Partition, Partition)>) -> f64 {
    partition_tuples.iter().filter(|&&(_, ref out_partition)| {
        out_partition.iter().find(|set| {
...
fn aggregated_out_out_probability(out_coins: &Vec<u64>, partition_tuples: &Vec<(Partition, Partition)>) -> (f64, f64, f64, f64) {
